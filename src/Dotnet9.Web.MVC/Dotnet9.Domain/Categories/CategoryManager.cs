...
public class CategoryManager
{
        ...
        int parentId)
    {
        ...
        if (existCategory != null) throw new Exception($"存在同名的分类: {name}");

        existCategory = await _categoryRepository.FindBySlugAsync(slug);
        if (existCategory != null) throw new Exception($"存在相同别名的分类: {slug}");

        if (id != null) return new Category(id.Value, name, slug, cover, description, parentId);
    ...
    public async Task ChangeNameAsync(Category category, string newName)
    {
        ...
        if (existCategory != null && existCategory.Id != category.Id) throw new Exception("存在同名的分类");
    ...
    public async Task ChangeSlugAsync(Category category, string newSlug)
    {
        ...
        if (existCategory != null && existCategory.Id != category.Id) throw new Exception("存在相同别名的分类");
