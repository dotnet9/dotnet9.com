...
const formModel = ref<AlbumItem>(defaultValue);
...
if (route.params.obj) {
  formModel.value = JSON.parse(route.params.obj.toString()) as AlbumItem;
}

onMounted(() => {});

const restore = () => {
  router.replace("/admin/editalbum");
  formModel.value = {
    name: "",
    slug: "",
    cover: "",
...
const save = async () => {
  try {
    ...
    loading.value = true;
    await http.post("/api/album/addOrUpdate", formModel.value);
    ElMessage({
      message: "保存成功",
      type: "success",
    });
